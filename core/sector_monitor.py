...
def get_leading_sub_themes(leading_sectors):
    """
    선정된 주도 섹터(대분류) 내 세부 테마 ETF들을 스캔하여
    각 섹터별 세부 테마의 모멘텀 지표 및 대파동 상태를 분석합니다.
    """
    print("\n🌐 [1단계-B: 세부 테마 분석] 주도 섹터 내 마이크로 테마 스캔 중...")
    
    sub_theme_results = {}
    
    for major_sector in leading_sectors:
        # 영어 대분류 섹터명 추출 (예: 'Technology (기술)' -> 'Technology')
        major_eng = major_sector.split('(')[0].strip()
        
        if major_eng not in config.SUB_SECTOR_CONFIG:
            continue
            
        themes = config.SUB_SECTOR_CONFIG[major_eng]
        theme_scores = []
        
        for theme_name, theme_info in themes.items():
            etfs = theme_info['etfs']
            desc = theme_info['desc']
            
            # 테마 내 대표 ETF (첫 번째 ETF를 대표로 분석)
            if not etfs:
                continue
            ticker = etfs[0]
            ...
            try:
                ...
                status = "🔥 파동상승" if curr_k > curr_d else "눌림/조정"
                ...
                # print(f"⚠️ {theme_name}({ticker}) 데이터 수집 실패: {e}")
                continue
                
        if theme_scores:
            # 스코어 기준 정렬
            theme_scores = sorted(theme_scores, key=lambda x: x['score'], reverse=True)
            sub_theme_results[major_sector] = theme_scores
            
            print(f"\n   [Sector: {major_sector} 하위 테마 순위]")
            for t in theme_scores:
                print(f"   - {t['theme_name']} ({t['ticker']}) | 모멘텀: {t['roc_1m']:.2f}% | 대파동: {t['status']} | 특징: {t['desc']}")
                
    return sub_theme_results


# 모듈 단독 테스트용 코드
if __name__ == "__main__":
    lead_names, lead_tickers = get_leading_sectors()
    get_leading_sub_themes(lead_names)
